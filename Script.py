...
data = load_dataset("imdb", split="train[:25000]")
...
input = tokenizer(test_sentence, return_tensors="pt", padding=True,
truncation=True)
# Running inference
with torch.no_grad():
...
# %%
# Sentence to run inference on
test_sentence = "The CGI was so realistic I almost believed those were actual cardboard cutouts."
# Tokenizing the sentence
input = tokenizer(test_sentence, return_tensors="pt", padding=True,
truncation=True)
# Running inference
...
tokenized_train = train_data.map(tok, batched=True)
tokenized_test  = test_data.map(tok, batched=True)
...
# Pick a device (CUDA if available; fall back to CPU; include MPS for Apple Silicon if you want)
device = (
...
test_sentence = "like this movie"
inputs = tokenizer(test_sentence, return_tensors="pt", truncation=True).to(device)

with torch.no_grad():
    logits = model(**inputs).logits
pred_class = int(torch.argmax(logits, dim=-1).item())
label = model.config.id2label.get(pred_class, {0: "negative", 1: "positive"}[pred_class])
print(f"Predicted sentiment is: {label.lower()}")
